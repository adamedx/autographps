...
function FindAssembly($assemblyRoot, $assemblyName) {
    $matchingAssemblyPaths = ls -r $assemblyRoot -Filter $assemblyName | sort lastwritetime | where {$components = $_.fullname -split "\\"; $components[$components.length - 2] -eq 'net45' }
